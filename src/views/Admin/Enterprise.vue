    ...
    <el-table border :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="Img" label="企业Logo">
        <template slot-scope="scope">
          <img style="height: 100px" :src="imgserver + scope.row.Img" alt />
        </template>
      </el-table-column>
    ...
    <el-dialog title="合作企业管理" :visible.sync="dialogFormVisible">
      <el-form :model="formData">
        <el-form-item label="企业Logo" :label-width="formLabelWidth">
          <Uploader :img="formData.Img" folder="enterprise_image" :updateImg="updateImg"></Uploader>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleCreateOrModify() {
      if (!this.formData.Id) {
        https
          .post('Admin/Enterprise/CreateEnterprise', this.formData)
          .then((response) => {
        ...
        https
          .put('Admin/Enterprise/ModifiedEnterprise', this.formData)
          .then((response) => {
            if (response.type === 'success') {
              this.$message({
                message: '修改成功！',
    ...
    handleDelete(index, row) {
      this.$confirm('此操作将永久此条数据, 是否继续?', '提示', {
        ...
        .then(() => {
          https
            .delete(`Admin/Enterprise/DeleteEnterprise?id=${row.Id}`)
            .then((response) => {
              if (response.type === 'success') {
                this.$message({
                  message: '删除成功！',
